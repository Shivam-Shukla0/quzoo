...
def debug_production():
    """Debug route to check production environment"""
    import os
    try:
        # Check environment variables
        has_api_key = bool(os.environ.get('GEMINI_API_KEY'))
        api_key_length = len(os.environ.get('GEMINI_API_KEY', '')) if has_api_key else 0
        
        # Test AI generation
        from api_utils import generate_quiz_questions
        test_result = generate_quiz_questions("Python", 1)
        ...
        return jsonify({
            ...
            "ai_test": {
                "success": True,
                "questions_generated": len(test_result),
                "sample_question": test_result[0] if test_result else None
            }
        })
    except Exception as e:
        return jsonify({
            ...
            "environment": {
                ...
                "api_key_length": len(os.environ.get('GEMINI_API_KEY', '')),
                "flask_env": os.environ.get('FLASK_ENV', 'not_set'),
                "debug": os.environ.get('DEBUG', 'not_set')
            },
            "ai_test": {
